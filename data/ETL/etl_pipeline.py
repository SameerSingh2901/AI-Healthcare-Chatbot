...
#             relations.append({
#                 "disease_id": d["uid"],
#                 "medicines_id": m["medicines_id"],
#             })

#         for p in d.get("precautions", []):
#             relations.append({
#                 "disease_id": d["uid"],
#                 "precautions_id": p["precautions_id"],
#             })

#     return diseases, relations, cures, medicines, precautions
...
#     save_csv("symptoms.csv", symptoms,
#              ["symptom_id", "name", "description", "body_site", "commonness"])
    
#     # Process cures
#     cures = process_cures(cures_raw)
#     save_csv("cures.csv", cures,
#              ["cures_id", "name", "description", "type"])
    
#     # Process medicines
#     medicines = process_medicines(medicines_raw)
#     save_csv("medicines.csv", medicines,
#              ["medicines_id", "name", "description", "drug_class", "dosage_form"])
    
#     # Process precautions
#     precautions = process_precautions(precautions_raw)
#     save_csv("precautions.csv", precautions,
#              ["precautions_id", "name", "description"])

#     # Process diseases & relations
#     diseases, relations = process_diseases(diseases_raw)
#     save_csv("diseases.csv", diseases,
#              ["disease_id", "name", "canonical_id", "description", "prevalence", "risk_factors"])
#     save_csv("disease_has_symptom.csv", relations,
#              ["disease_id", "symptom_id", "weight"])
#     save_csv("disease_has_cures.csv", relations,
#              ["disease_id", "symptom_id", "weight"])
#     save_csv("disease_has_medicines.csv", relations,
#              ["disease_id", "symptom_id", "weight"])
#     save_csv("disease_has_precautions.csv", relations,
#              ["disease_id", "symptom_id", "weight"])
...
def process_diseases(diseases_raw):
    ...
    for d in diseases_raw:
        ...
        for c in d.get("cures", []):
            cure_relations.append({
                "disease_id": d["uid"],
                "cure_id": c
            })

        for m in d.get("medicines", []):
            medicine_relations.append({
                "disease_id": d["uid"],
                "medicine_id": m
            })

        for p in d.get("precautions", []):
            precaution_relations.append({
                "disease_id": d["uid"],
                "precaution_id": p
            })

    return diseases, symptom_relations, cure_relations, medicine_relations, precaution_relations
...
def run_etl():
    ...
    # Process diseases & relations
    diseases, symptom_relations, cure_relations, medicine_relations, precaution_relations = process_diseases(diseases_raw)
    ...
    # Save relations
    save_csv("disease_has_symptom.csv", symptom_relations,
             ["disease_id", "symptom_id", "weight"])
    save_csv("disease_has_cure.csv", cure_relations,
             ["disease_id", "cure_id"])
    save_csv("disease_has_medicine.csv", medicine_relations,
             ["disease_id", "medicine_id"])
    save_csv("disease_has_precaution.csv", precaution_relations,
